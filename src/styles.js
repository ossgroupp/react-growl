...
const GrowlComponent = styled.div.attrs(({ type }) => {
  return {
    className: `osspim-growl__item osspim-growl__item-${
      type || 'info'
    }`,
  };
})`
  ...
  &.osspim-growl__item-error {
    background: #facbcf;
    color: #967376;
  }

  &.osspim-growl__item-warning {
    background: #fdf5bf;
    color: #9a946e;
  }
...
const Text = styled.div.attrs(() => ({
  className: 'osspim-growl__item-text',
}))`
  margin-top: 5px;
  font-size: 14px;
`;
...
const Title = styled.strong.attrs(() => ({
  className: 'osspim-growl__item-title',
}))`
  display: block;
  font-size: 16px;
`;
...
const RemoveButton = styled.button.attrs(() => ({
  className: 'osspim-growl__item-remove',
}))`
  appearance: none;
  border: none;
  background: transparent;
